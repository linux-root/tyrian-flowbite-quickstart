...
object ComponentsDemo:
  ...
  private def componentItem(name: String, page: Page, imageUrl: String): Html[Msg] =
    ...
    a(
      href := page.path,
      cls  := "block h-64 bg-white rounded-lg shadow-md transition-transform duration-300 ease-in-out hover:scale-105 border border-gray-100 hover:border-white dark:border-gray-800 dark:hover:border-gray-700 hover:shadow-lg dark:hover:shadow-lg dark:bg-gray-900"
    )(
      div(
        ...
        span(cls := "text-base font-medium text-gray-900 dark:text-white")(name),
        span(cls := "text-gray-500 dark:text-gray-400")(Icons.arrowRight)
      ),
  ...
  def apply(): Html[Msg] =
    ...
    val buttons    = componentItem("Buttons", Page.Buttons, "/assets/images/buttons")
    ...
    div(cls := "w-full max-w-4xl grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-3 mt-8")(
      components
    )

...
webpackDevServer := {
  val compiledScalajsFile = Paths.get(s"target/scala-${scalaVersion.value}/tyrian-flowbite-quickstart-fastopt/main.js")
...
lazy val publishDist = taskKey[Unit]("Build static web artifact")

publishDist := {
  val ignored = (Compile / fullLinkJS).value
  installNpmPackages()
  println(s"\n$GREEN Creating dist...$RESET")
  "npm run build:prod".!
}
